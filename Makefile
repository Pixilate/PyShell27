...
INCLUDES	:=  inc
APP_TITLE       :=  PyShell27
APP_DESCRIPTION :=  Python27 Shell
APP_AUTHOR      :=  MarcusD
APP_PRODUCT_CODE:=  CTR-P-APYA
APP_UNIQUE_ID   :=  0x11056
...
LIBS	:= -l:libpython2.7.a -lz -lctru -lm
...
export _3DSXFLAGS += --romfs=$(CURDIR)/assets/ROMFS
...
$(TOPDIR)/assets/banner.bin: $(TOPDIR)/assets/banner.png $(TOPDIR)/assets/banner.wav
	@bannertool makebanner -v visible,nosavebackups -i $(TOPDIR)/assets/banner.png -a $(TOPDIR)/assets/banner.wav -o $(TOPDIR)/assets/banner.bin

$(TOPDIR)/assets/image.bin: $(TOPDIR)/assets/logo.png
	@bannertool makesmdh -v visible,nosavebackups -s "$(APP_TITLE)" -l "$(APP_DESCRIPTION)" -p "$(APP_AUTHOR)" -i $(TOPDIR)/assets/logo.png -o $(TOPDIR)/assets/image.bin
...
$(OUTPUT).cia: stripped.elf $(TOPDIR)/assets/banner.bin $(TOPDIR)/assets/image.bin
	@makerom -f cia -o $(OUTPUT).cia -rsf $(TOPDIR)/assets/cia.rsf -target t -exefslogo -elf stripped.elf -icon $(TOPDIR)/assets/image.bin -banner $(TOPDIR)/assets/banner.bin -DAPP_TITLE="$(APP_TITLE)" -DAPP_PRODUCT_CODE="$(APP_PRODUCT_CODE)" -DAPP_UNIQUE_ID="$(APP_UNIQUE_ID)" -DAPP_ROMFS="$(TOPDIR)/assets/ROMFS"
	@echo "built ... $(notdir $@)"
